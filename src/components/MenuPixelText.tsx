...
export const MenuPixelText: React.FC<Props> = ({ 
  ...
  useEffect(() => {
    ...
    if (isMobile || !canvasRef.current || !containerRef.current) {
      if (isMobile) setWebglSupported(false);
      return;
    }

    try {
      const canvas = canvasRef.current;
      const gl = canvas.getContext("webgl", { alpha: true }) || canvas.getContext("experimental-webgl", { alpha: true }) as WebGLRenderingContext | null;
      ...
      console.warn("WebGL initialization failed, falling back to text rendering:", e);
      setWebglSupported(false);
    }
  }, [text, activeColor]);
  ...
  return (
    <div ref={containerRef} className={`relative flex items-center justify-center ${className}`}>
      {webglSupported ? (
        ...
        <span className="opacity-100 select-text">{text}</span>
      )}
    </div>
  );
};
